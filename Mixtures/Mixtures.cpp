...
int color(vi arr, int si, int ei) {
	int sum = 0;
	for (int i = si; i <= ei; i++) {
		sum += arr[i];
	}
...
int mixturesTD(vi arr, int si, int ei) {
	if (si == ei) {
		return 0;
	}
...
int main() {
	vi arr = {41, 67, 34, 0, 69, 24, 78, 58, 62, 64};
	cout << mixturesTD(arr, 0, int(arr.size()) - 1) << endl;
	cout << mixturesBU(arr) << endl;
}